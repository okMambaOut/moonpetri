...
实际尝试 `moon test --target native`，失败原因：`no system C compiler found; tried cl, cc, gcc, clang`。这是构建环境缺失，不是成功的 native 测试。GitHub 仓库现已重建并推送；远程运行 https://github.com/okMambaOut/moonpetri/actions/runs/34370073300 已成功完成四目标检查、构建、各 28 项测试、各 13 项 CLI 调用和 API demo，包含 native。这不改变上表的本机验证范围。
